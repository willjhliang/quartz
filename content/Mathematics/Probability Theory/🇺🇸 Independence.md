Two random variables $X$ and $Y$ are independent if and only if $$p(x, y) = p(x)p(y).$$ Intuitively, this is saying that if we know $y$, $x$ does not change. Using this equation, we also get the following properties.
1. $$p(y \vert x) = p(y)$$
2. $$p(x \vert y) = p(x)$$
